...
class Ticket extends React.Component {
  ...
  componentDidMount() {
     ...
     const interval = setInterval(() => {
        const maxInstrumentsForDemo = 10;
        if(this.props.marketfeed.instruments.length > 0) {
            clearInterval(interval);
            for(let i=0; i<maxInstrumentsForDemo; i++) {
                this.props.marketfeed.instruments.forEach(instrument => {
  ...
  symbolChange(e) {
     this.setState( {
        price: this.props.marketfeed.selected.price.toFixed(2)
     });
  }
  ...
  render() {
    ...
    let bidPrice = null;
    let askPrice = null;
    let instrument = marketfeed.selected;
