...
describe("InstrumentsMarketfeed", () => {
  ...
  beforeEach(() => {
    props = {
      marketfeedInstruments: [
        {
          delete: <img id="deleteImg-AMZ" src="img/delete.png" alt="delete"/>,
          symbol: "AMZ"
        }, {
  ...
  it("should call selectMarketfeedInstrument if not already selected", () => {
    ...
    clickAndVerify(component,
                  "div[children='" + selectInstrument.symbol + "']",
  ...
  it("should not call selectMarketfeedInstrument if already selected", () => {
    clickAndVerify(component,
                   "div[children='" + props.instrument.symbol + "']",
  ...
  it("should alert cannot delete when there is only one instrument", () => {
    const selectedMrkInstrument = props.marketfeedInstruments.find(
                                    instrument => instrument.symbol === props.instrument.symbol);
    component = mount(<InstrumentsMarketfeed { ...props} marketfeedInstruments={[selectedMrkInstrument]}/>);

    clickAndVerify(component,
                   "img#" + getDeleteId(selectedMrkInstrument),
                   () => expect(window.alert)
                            .toHaveBeenCalledWith(expect.stringMatching(/at least one/)));
  ...
  it("should delete and select first marketfeed when current select is not first", () => {
    ...
    const selectedMrkInstrument = props.marketfeedInstruments.find(
                                      instrument => instrument.symbol !== firstMarketfeedInstrument.symbol);
  ...
  it("should delete and select next marketfeed when current select is first", () => {
    const firstMarketfeedInstrument = props.marketfeedInstruments[0];
    const nextMrkInstrument = props.marketfeedInstruments.find(
                                instrument => instrument.symbol !== firstMarketfeedInstrument.symbol);
  ...
  function deleteAndSelect(component, selectedMrkInstrument, nextSelectedInstrument) {
    clickAndVerify(component, "img#" + getDeleteId(selectedMrkInstrument), () => {
      ...
      expect(props.deleteMarketfeedInstrument)
          .toHaveBeenCalledWith(expect.objectContaining({symbol: selectedMrkInstrument.symbol}));

      expect(props.selectMarketfeedInstrument)
          .toHaveBeenCalledWith(expect.objectContaining({symbol: nextSelectedInstrument.symbol}));
    });
  }
