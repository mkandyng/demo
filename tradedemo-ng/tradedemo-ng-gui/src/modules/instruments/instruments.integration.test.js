...
import {MAX_INSTRUMENTS, retrieveInstrument, getDeleteId} from "./instruments";
...
describe("InstrumentsSearch integration tests", () => {
  ...
  it("should add to marketfeedInstruments and remove from instruments", () => {
    ...
    addInstrumentToMarketfeedVerify(instrumentsToAdd.symbol, {
      ...
      expect(retrieveInstrument(store.getState().searchInstruments, instrumentsToAdd.symbol)).not.toBeDefined();
      expect(retrieveInstrument(store.getState().marketfeedInstruments, instrumentsToAdd.symbol)).toBeDefined();
    });
  });
...
describe("InstrumentsMarketfeed integration tests", () => {
  ...
  it("should select another instrument if not already selected", () => {
    const selectInstrument = getNextSelectedMarketfeed(props.marketfeedInstruments, props.instrument);
  ...
  it("should delete and select next marketfeed", () => {
    ...
    clickAndVerify(component, "img#" + getDeleteId(selectedMrkInstrument), () => {
      const nextSelectedInstrument = getNextSelectedMarketfeed(props.marketfeedInstruments, selectedMrkInstrument);

      expect(retrieveInstrument(store.getState().marketfeedInstruments, selectedMrkInstrument.symbol)).not.toBeDefined();

      expect(retrieveInstrument(store.getState().searchInstruments, selectedMrkInstrument.symbol)).toBeDefined();
...
function createStoreAndFetchInstruments() {
  const instruments = createInstruments(MAX_INSTRUMENTS + 1).map(instrument => ({
    ...instrument,
    price: 1.0,
    open: 1.0
  }));
  ...
  }, instrumentsReducer, instrumentsEpics);
  ...
  const props = {
    ...
    addInstrumentToMarketfeed: instrument => store.dispatch(addInstrumentToMarketfeed(instrument)),
    selectMarketfeedInstrument: instrument => store.dispatch(selectMarketfeedInstrument(instrument)),
    updateMarketfeedInstrument: instrument => store.dispatch(updateMarketfeedInstrument(instrument)),
    deleteMarketfeedInstrument: instrument => store.dispatch(deleteMarketfeedInstrument(instrument))
  };

  return ({instruments: instruments, store: store, props: props});
}
