...
import {addInstrumentToMarketfeedVerify, createInstruments} from "../instruments.test.helpers";
...
describe("InstrumentsSearch", () => {
  ...
  it("should update value on input change", () => {
    ...
    // When
    component.find("input").find({role: "combobox"}).simulate("change", {
      target: {
        value: expectedValue
      }
    });
    ...
    // Then
    expect(component.find("input").find({value: expectedValue}).props().value).toEqual(expectedValue);
  });

  it("should reject input not in instruments", () => {
    addInstrumentToMarketfeedVerify("invalid",
                                    props, () => expect(window.alert)
