...
describe("App Integration Test - Standard tests", () => {
    ...
    let store = undefined;
    window.confirm = () => true;
    const instruments = createInstruments(MAX_INSTRUMENTS)
    ...
    it("should validate redux store is working as expected on init", () => {
        ...
        expect(instruments.searchInstruments)
                .toHaveLength(MAX_INSTRUMENTS-MAX_MARKET_FEED_INSTRUMENTS);
        expect(instruments.marketfeedInstruments)
                .toHaveLength(MAX_MARKET_FEED_INSTRUMENTS);
    ...
    it("should add an instrument from searchInstruments to marketfeed", () => {
        ...
        // Given
        const instrumentInSearch = {...store.getState().instruments.searchInstruments[0]};
        const instrumentSearchInput = component.find(InstrumentsSearch).find({role: "combobox"});
        instrumentSearchInput.simulate("change", {target: {value: instrumentInSearch.symbol}});
        ...
        expect(store.getState()
                    .instruments
                    .marketfeedInstruments[0]).toEqual(expect.objectContaining({
                                                              symbol: instrumentInSearch.symbol
                                                       }));
    });
    ...
    it("should submit ticket value to orderbook when buying and selling", () => {
        ...
        const verify = (buySellId, buySell, orderId) => {
            const nextSelect = getNextSelectedMarketfeed(
                                  store.getState().instruments.marketfeedInstruments,
...
describe("App Integration Test - handling marketfeed instrument select", () => {
    ...
    beforeEach(() => {
        ...
        act(() => {
            component = mount(appWithProvider(store));
            nextSelect = getNextSelectedMarketfeed(
                                  store.getState().instruments.marketfeedInstruments,
                                  store.getState().instruments.selected);
        });
    });
    ...
    it("should update intraday prices tab for selected instrument", () => {
        clickAndVerify(component.find(InstrumentsMarketfeed),
            ...
            expect(component.find(Tab)
                            .find("li[children='" + matchText + "']").text())
                            .toMatch(matchText);
    ...
    it("should update ticket prices if selected marketfeed instrument prices change", () => {
        clickAndVerify(component.find(InstrumentsMarketfeed),
                      "div[children='" + nextSelect.symbol + "']", () => {
            const select = { ...nextSelect, bidPrice: "1.99", askPrice: "1.95" };
